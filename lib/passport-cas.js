 ...
 *   - `serverBaseURL`     Base URL of the our server (e.g. http://localhost:1337)
 *   - `serviceURL`        URL fragment for service url (e.g. /auth/cas)
...
function CasStrategy(options, verify) {
  ...
  this.fullAttributes = options.fullAttributes || false;
  ...
  if (!ticket) {
    ...
    self.cas.validate(ticket, function(err, status, username, extended) {
      ...
      // Validation successful
        ...
        if (self.casPgtUrl && extended && extended.PGTIOU) {
          req.session[self.casSessionKey].PGTIOU = extended.PGTIOU;
        }

        var attributes = extended && extended.attributes ? extended.attributes : {};
        var profile = self.fullAttributes ? {} : {
          provider: 'CAS',
          id: (extended && extended.id) || username,
        ...
        for (var key in attributes) {
          var mappedKey = self.casPropertyMap[key] || key;
          var value = attributes[mappedKey];
          // if attribute has more values then consider it as array
