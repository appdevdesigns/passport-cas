 ...
 *   - `pgtURL`            URL of the PGT callback server (e.g. https://callback.example.com)
 *   - `sessionKey`        The name to use for storing CAS information within the `req.session` object. Default is 'cas'.
 ...
 *      The 3rd party services you request may need to add this URL as a trusted
 *      proxy as well.
 ...
 * @api public
 */
function PgtServer(casURL, pgtURL, serverCertificate, serverKey) {
  var parsedURL = url.parse(pgtURL);
  var cas = new CAS({
    base_url: casURL,
    version: 2.0,
    proxy_server: true,
    proxy_callback_host: parsedURL.hostname,
    proxy_callback_port: parsedURL.port,
    proxy_server_key: options.serverKey,
    proxy_server_cert: options.serverCertificate
  });
}
